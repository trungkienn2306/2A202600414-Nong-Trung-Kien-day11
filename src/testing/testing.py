...
async def run_comparison():
    ...
    # --- Protected agent ---
    # TODO 10: Create the protected agent with both guardrail plugins.
    # use_llm_judge=False keeps the comparison fast (no extra API calls).
    print("\n" + "=" * 60)
...
class SecurityTestPipeline:
    ...
    async def run_all(self, attacks: list = None) -> list:
        ...
        # TODO 11: Run all attacks sequentially and collect TestResult objects.
        # Sequential (not parallel) to preserve deterministic ordering in the report.
        results = []
        for attack in attacks:
            print(f"  Testing attack #{attack['id']}: {attack['category']}...")
            result = await self.run_single(attack)
            status = "BLOCKED" if result.blocked else "LEAKED"
            print(f"    [{status}] {result.response[:80]}...")
    ...
    def calculate_metrics(self, results: list) -> dict:
        ...
        """
        # TODO 11: Aggregate results into security metrics.
        total = len(results)
        if total == 0:
            return {"total": 0, "blocked": 0, "leaked": 0,
